...
pyprojectInstallPhase() {
  ...
  for wheel in ./*.whl; do
    @uv@/bin/uv pip --offline --no-cache install --no-deps --link-mode=copy --python=@pythonInterpreter@ --python-platform=aarch64-unknown-linux-gnu --system --prefix "$out" $uvPipInstallFlags "$wheel"
